...
class DBN:
    ...
    def predict(self,
        ...
        Args:
            x_test: Input RGB data (shape: (n_samples, 3))
            batch_size: Batch size for prediction
            
        Returns:
            Predicted CIELAB values (shape: (n_samples, 3))
        """
...
class PSOOptimizer:
    ...
    def optimize(self,
        ...
        Args:
            dbn: DBN model instance
            x_train: Scaled RGB training data
            y_train: Scaled CIELAB training data
            x_val: Validation data (optional)
            y_val: Validation labels (optional)
            
        Returns:
            Optimized weights for the DBN
        """
    ...
    def _calculate_adaptive_bounds(self, weights: List[np.ndarray]) -> List[Tuple[float, float]]:
        ...
        for w in weights:
            ...
            if std_w > 0:
                # use 3 sigma rule for bounds
                margin = 3 * std_w
                min_val = mean_w - margin
                max_val = mean_w + margin
                
            else:
                # fallback for constant weights
    ...
    def _validate_weights(self, 
        ...
        try:
            for opt_w, init_w in zip(optimized_weights, initial_weights):
                ...
                if np.abs(opt_w).max() > 10 * np.abs(init_w).max():
                    return False
...
def optimize_dbn_with_pso(dbn: DBN,
    ...
    Returns:
        Tuple of (optimized_weights, optimization_metrics)
    """
...
def convert_colors_to_cielab_dbn(dbn: DBN, 
    ...
    Returns:
        CIELAB colors predicted by DBN as list of tuples
    """
    if not avg_colors:
    ...
    try:
        logger.info(f"Converting {len(avg_colors)} RGB colors to CIELAB using PSO-DBN")
        
        # Convert and validate RGB colors
        ...
        if avg_colors_array.max() <= 1.0:
            ...
            logger.debug("Converted RGB colors from 0-1 to 0-255 range")
        ...
        logger.info("Color conversion using PSO-DBN completed successfully")
        return result
        
    except Exception as e:
        logger.error(f"DBN color conversion failed: {e}")
        # Return neutral gray as fallback
        fallback_color = (50.0, 0.0, 0.0)  # Neutral gray in CIELAB
